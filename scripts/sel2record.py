...
def main():
    ...
    parser.add_argument('-m', "--model_path", dest='tokenizer_path', help='The path of tokenizer')
    options = parser.parse_args()

    map_config = MapConfig.load_from_yaml(options.map_config)

    schema_dict = SEL2Record.load_schema_dict(options.gold_folder)
    tokenizer = AutoTokenizer.from_pretrained(options.tokenizer_path) if options.tokenizer_path else None
    ...
    for pred_folder in options.pred_folder:
        ...
        for data_key, (generation, gold_file, record_file) in data_dict.items():
            ...
            gold_list = [json.loads(line) for line in open(gold_filename, "r", encoding="utf-8")]
            gold_text_list = [gold['text'] for gold in gold_list]
            gold_token_list = [gold['tokens'] for gold in gold_list]

            pred_list = [line.strip() for line in open(pred_filename, "r", encoding="utf-8").readlines()]

            assert len(gold_text_list) == len(pred_list)

            pred_records = list()
            for i, (pred, text, tokens) in enumerate(zip(pred_list, gold_text_list, gold_token_list)):
                pred_record = sel2record.sel2record(pred, text, tokens)
                pred_records += [pred_record]

            with open(os.path.join(pred_folder, record_file), 'w', encoding="utf-8") as output:
